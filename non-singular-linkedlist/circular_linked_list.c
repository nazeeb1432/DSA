...
void len(){
    length=0;
    struct node *ptr = tail->next;//i.e from head;
    while(ptr->next != tail->next){
       ptr = ptr->next;
       length++;
    }
...
void creating_list(int data) {
   ...
   tail->next=head;//link ke point korar por
...
void insertbeg(){
    ...
    if(tail==0){
        tail=link;
        tail->next=link;//single element selg loop
        //head=link;
    }else{
        link->next=tail->next;//tail->next is head;
        tail->next=link;
        //head=link;
    }
