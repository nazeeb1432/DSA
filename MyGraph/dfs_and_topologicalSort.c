...
void dfs(int vertice){
    int i;
    for(i=0;i<vertice;i++){//shob vertex ke initialize kore dilam
        previous[i]=-1;
        finish[i]=__INT_MAX__;
        discovery[i]=__INT_MAX__;
    }
    for(i=0;i<vertice;i++){//prottekta vertex ke visit korbo;
        if(color[i]==0){//for forest ,i=1,2,3,4,5 er jonno color black pabe,for i=6(f) er jonno dhukbe
...
void dfs_visit(int vertice,int source){
    ...
    for(i=0;i<vertice;i++){
        if(graph[source][i]==1  && color[i]==0){
            previous[i]=source;
            dfs_visit(vertice,i);
        }
    }
    color[source]=2;
    push(source);//for topological sort,black holei insert into stack
    finish[source]=++time;
}